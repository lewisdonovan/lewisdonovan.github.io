...
class X {
  #config: XConfig;
  #postprocessing: { setSize?: (w: number, h: number) => void; dispose?: () => void; render?: () => void } | null = null;
  ...
  set postprocessing(value: { setSize?: (w: number, h: number) => void; dispose?: () => void; render?: () => void } | null) {
  ...
  clear() {
    this.scene.traverse(obj => {
      const mesh = obj as { isMesh?: boolean; material?: { dispose?: () => void; [key: string]: unknown } | null; geometry?: { dispose?: () => void } };
...
class Y extends MeshPhysicalMaterial {
  ...
  onBeforeCompile2?: (shader: { uniforms: Record<string, { value: unknown }>; fragmentShader: string }) => void;
}

const XConfig = {
  count: 200,
  // Emerald green theme colors (as hex numbers)
  colors: [0x10b981, 0x059669, 0x34d399, 0x6ee7b7, 0x047857],
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, colors, ...props }) => {
  ...
  return <canvas className={`${className} w-full h-full absolute inset-0`} ref={canvasRef} />;
};

export default Ballpit;
export { Ballpit as BallPit };
