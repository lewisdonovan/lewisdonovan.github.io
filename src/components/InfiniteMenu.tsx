...
void main() {
    // Get original UV coordinates for circular mask
    vec2 uv = vec2(vUvs.x, 1.0 - vUvs.y);
    
    // Hard circular mask - discard pixels outside the circle
    ...
    // Hard crop: scale to fill the container, cropping excess
    // For 16:9 images, this will crop the sides (horizontal crop)
    // For 9:16 images, this will crop the top/bottom (vertical crop)
    float scale = min(imageAspect / containerAspect, 
                     containerAspect / imageAspect);
    
    // Apply crop scaling - this will zoom in and crop to fill
    vec2 st = uv;
    st = (st - 0.5) / scale + 0.5;
    ...
    // Reduce opacity for non-active items
    float opacityMultiplier = 1.0;
    if (uActiveItemIndex >= 0 && itemIndex != uActiveItemIndex) {
        opacityMultiplier = 0.25;
    }
    
    outColor.a *= vAlpha * opacityMultiplier;
}
`;
...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Draw image centered with preserved aspect ratio
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      ...
      // Reset active item during drag so all items are visible
      this.activeItemIndex = -1;
      cameraTargetZ += this.control.rotationVelocity * 40 + 1.5;
      damping = 7 / timeScale;
    }
        ...
        `}
          ...
          ${
            isMoving
              ...
              : 'bottom-0 opacity-100 pointer-events-auto duration-[500ms] scale-100 -translate-x-1/2'
          }
        `}
          >
